...
Para o fine-tuning, utilizamos o \textbf{MedQuAD} (Medical Question Answering Dataset), um dataset de perguntas e respostas médicas criado pelo National Institutes of Health (NIH) dos Estados Unidos.
...
\begin{tabular}{lccc}
\toprule
\textbf{Dataset (HuggingFace)} & \textbf{Pares QA} & \textbf{Metadados} & \textbf{Escolhido} \\
\midrule
\texttt{lavita/MedQuAD} & 47.400 & UMLS, fontes, sinônimos & \checkmark \\
\texttt{keivalya/MedQuad-MedicalQnADataset} & 16.400 & Apenas tipo de questão & \\
\texttt{Tonic/medquad} & 15.549 & Nenhum & \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{Maior volume}: 47.400 pares QA (quase 3x mais que alternativas)
...
O dataset original do MedQuAD compila informações de 12 fontes do NIH, cobrindo diversas especialidades médicas:
...
\begin{tabular}{llc}
\toprule
\textbf{Fonte} & \textbf{Cobertura} & \textbf{Pares QA} \\
\midrule
National Cancer Institute (NCI) & Oncologia & 729 \\
Genetics Home Reference (GHR) & Genética & 5.430 \\
MedlinePlus Health Topics & Saúde geral & 981 \\
NIDDK & Diabetes, digestivo, renal & 1.192 \\
NINDS & Neurologia & 1.104 \\
NIHSeniorHealth & Geriatria & 769 \\
NHLBI & Cardiologia, pneumologia & 559 \\
CDC & Doenças infecciosas & 270 \\
\midrule
\textbf{Total (versão filtrada)} & & \textbf{47.400} \\
\bottomrule
\end{tabular}
\end{table}
...
Dado o volume do dataset (47.400 registros), implementamos um sistema de checkpoints que salva o progresso a cada 1.000 traduções. Isso permite:
...
Uma limitação crítica do Google Colab Free é o limite de sessão de aproximadamente 12 horas. Para um treino de 5 epochs com 47.400 registros, o tempo total estimado é de 20-30 horas, exigindo múltiplas sessões.
...
\midrule
47.400 pares & 1 & 4-6 horas & 1 \\
47.400 pares & 3 & 12-18 horas & 2-3 \\
\textbf{47.400 pares} & \textbf{5} & \textbf{20-30 horas} & \textbf{3-4} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{Dataset}: MedQuAD com 47.400 pares QA, traduzido para português via \texttt{deep-translator}
    \item \textbf{Técnica}: QLoRA 4-bit ($r=64$, $\alpha=128$), permitindo treino em GPU T4 (15GB)
    \item \textbf{Treinamento}: 5 epochs, ~20-30h, dividido em 3-4 sessões do Colab Free
