...
O treinamento foi executado em servidor local com GPU NVIDIA RTX A5000 (24GB VRAM). A Tabela~\ref{tab:resultados-treino} apresenta os resultados obtidos.
...
\midrule
Steps totais & 5.780 \\
Loss final (treino) & 6,79 \\
Tempo total & $\sim$55 horas \\
...
\begin{table}[H]
\centering
\caption{Tempo de treinamento por configuração}
\label{tab:tempo-treino}
\begin{tabular}{ccccc}
\toprule
\textbf{Ambiente} & \textbf{GPU} & \textbf{Epochs} & \textbf{Tempo} & \textbf{Sessões} \\
\midrule
\textbf{Local} & \textbf{RTX A5000 (24GB)} & \textbf{5} & \textbf{$\sim$55h} & \textbf{1} \\
Colab Free & T4 (15GB) & 5 & 10-15h & 2 \\
...
O modelo resultante é publicado no Hugging Face Hub e utilizado como o ``cérebro'' do agente de raciocínio no grafo multi-agente descrito no Capítulo~\ref{cap:multiagente}.
