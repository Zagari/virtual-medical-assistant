...
\midrule
\multirow{2}{*}{Triagem}
    & \texttt{query\_type} & \texttt{Literal} & Tipo: protocolo, paciente, ambos ou fora\_de\_escopo \\
...
\begin{enumerate}
    \item \textbf{Triagem} $\rightarrow$ tipo ``ambos'', entidades: paciente=Maria Santos, condição=diabetes
    ...
    \item \textbf{Explicabilidade} $\rightarrow$ Confiança alta, fontes: Diabetes, Hipertensão, Exame HbA1C
...
\begin{enumerate}
    \item \textbf{Estado compartilhado vs.\ mensagens:} Optamos por estado compartilhado ao invés de troca de mensagens entre agentes. Isso simplifica o fluxo e permite que cada agente acesse o contexto completo sem repropagação.
    ...
    \item \textbf{LLM injetável com cascata de fallback:} A LLM é passada via parâmetro ao invés de ser instanciada dentro do grafo. No startup, o sistema tenta carregar o melhor modelo disponível em cascata:

    \begin{enumerate}
        \item \textbf{Fine-Tuned LoRA} (path local ou Hugging Face Hub) --- requer CUDA;
        \item \textbf{Fine-Tuned GGUF} (arquivo quantizado $\sim$4~GB) --- funciona em CPU e Apple Silicon;
        \item \textbf{Mistral 7B baseline} (Hugging Face Hub) --- requer CUDA;
        \item \textbf{Sem LLM} --- respostas baseadas apenas nos protocolos recuperados.
    \end{enumerate}

    A detecção é automática: se CUDA está disponível, carrega via \texttt{transformers}; caso contrário, tenta GGUF via \texttt{llama-cpp-python}. Ao trocar de modelo, o anterior é liberado da memória para evitar dois modelos simultâneos na RAM/VRAM.
...
\section{Interface Gradio}\label{sec:interface}

A interface do assistente (\texttt{app/main.py}) é construída com Gradio e expõe os seguintes componentes:

\begin{itemize}
    \item \textbf{Consulta médica:} campo de texto livre para a pergunta do médico;
    \item \textbf{Paciente:} dropdown filtrável com os 80 pacientes do banco (preenchido via PostgreSQL ou fallback JSON);
    \item \textbf{Seletor de Modelo LLM:} controle Radio com as opções disponíveis na máquina atual (Fine-Tuned, GGUF, Baseline ou Sem LLM), com detecção automática de hardware;
    \item \textbf{Status do modelo:} campo que exibe qual modelo está ativo e mensagens de carregamento/troca;
    \item \textbf{Pipeline de Agentes:} accordion expansível que exibe o \textit{report} de cada agente durante a execução, permitindo ao usuário visualizar o que cada etapa do grafo processou: classificação da triagem, dados do paciente recuperados, protocolos encontrados, resumo do raciocínio, resultado dos guardrails e métricas de explicabilidade;
    \item \textbf{Resposta:} saída formatada com resposta clínica, nível de confiança (alta/média/baixa) e fontes consultadas;
    \item \textbf{Log de auditoria:} accordion expansível com o JSON completo do trace de execução.
\end{itemize}

O \textbf{Pipeline de Agentes} foi projetado para fins de transparência e demonstração do funcionamento interno do sistema multi-agente. Cada agente adiciona um relatório amigável ao campo \texttt{agent\_reports} do estado compartilhado, que é então formatado em Markdown e exibido em seções separadas por agente. Um exemplo de visualização do pipeline:

\begin{lstlisting}[caption={Exemplo de report do Pipeline de Agentes na interface Gradio.}]
### TRIAGEM
**Tipo:** Protocolo + Paciente
...
**Paciente:** Maria Santos
**Comorbidades:** hipertensao, diabetes tipo 2
Dados encontrados: 3 exames, 2 prontuarios

### PROTOCOLOS
**5 protocolo(s) encontrado(s):**
Relevancia Direta:
  - Pathway-Diabetes | Tratamento (dist: 0.23)
  - Pathway-Hipertensao (dist: 0.31)

### RACIOCINIO
**Resposta gerada:** 187 palavras
**Contexto utilizado:** 5 protocolos + dados do paciente

### GUARDRAILS
**Resultado: APROVADO**
- Sem prescricao direta
- Validacao humana mencionada
- Sem diagnostico definitivo
...
Ao trocar o modelo pelo seletor, o sistema descarrega o anterior da memória e carrega o novo sob demanda (\textit{lazy loading}), garantindo que apenas um modelo ocupe memória por vez.
