...
\begin{enumerate}
    ...
    \item \textbf{LLM injetável com cascata de fallback:} A LLM é passada via parâmetro (\texttt{run\_assistant(query, llm=...)}) ao invés de ser instanciada dentro do grafo. No startup, o sistema tenta carregar o melhor modelo disponível em cascata:
    ...
    A detecção é automática: se \texttt{torch.cuda.is\_available()} retorna \texttt{True}, carrega via \texttt{transformers}; caso contrário, tenta GGUF via \texttt{llama-cpp-python}. Ao trocar de modelo, o anterior é liberado da memória (\texttt{del model} + \texttt{gc.collect()} + \texttt{torch.cuda.empty\_cache()}) para evitar dois modelos simultâneos na RAM/VRAM.
