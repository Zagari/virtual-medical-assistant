...
\begin{itemize}
    \item \textbf{Busca contextualizada:} Quando dados do paciente estão disponíveis (fluxo ``ambos''), o agente realiza \textbf{uma busca por comorbidade} do paciente (ex: uma busca para ``hipertensão'' e outra para ``diabetes tipo 2''), garantindo cobertura equilibrada. Sem dados do paciente, a busca é ampla com a query original;
...
\begin{infobox}[Por que busca por comorbidade?]
Uma única busca semântica com query longa tende a favorecer uma comorbidade sobre outra. Para um paciente com hipertensão e diabetes, a busca unificada retornava quase exclusivamente protocolos de hipertensão. A busca separada (3 resultados por comorbidade, deduplicados) garante representatividade de cada condição.
\end{infobox}
...
\begin{enumerate}
    ...
    \item \textbf{Protocolo} $\rightarrow$ Busca por comorbidade: ``exames + hipertensão'' e ``exames + diabetes'' $\rightarrow$ 3 Pathway-Hipertensão + 3 Pathway-Diabetes (todos ``direta'')
...
\begin{enumerate}
    ...
    \item \textbf{Paciente antes de protocolo:} Para queries que envolvem um paciente específico (tipo ``ambos''), o grafo busca os dados do paciente \emph{antes} dos protocolos. Isso permite que a busca no ChromaDB seja contextualizada pelas comorbidades reais do paciente, evitando resultados irrelevantes. A busca é feita separadamente por comorbidade para garantir cobertura equilibrada.
\end{enumerate}
