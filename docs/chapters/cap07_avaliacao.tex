...
Este capítulo apresenta a avaliação comparativa completa do fine-tuning. Sete checkpoints de quatro sessões de treinamento foram avaliados em duas trilhas complementares, revelando o ponto ótimo do modelo e um fenômeno de colapso catastrófico nas épocas intermediárias. Uma sessão adicional com \textit{early stopping} foi conduzida para validar a metodologia recomendada e comparar com o melhor checkpoint identificado.
...
ES         & early-stopping   & 1.000 & 0,9 & Sessão 4 (do zero, ES) \\
\bottomrule
\end{tabular}
\end{table}

\begin{warningbox}[Nota Metodológica: Sessões de Treinamento Distintas]
Os checkpoints avaliados provêm de \textbf{quatro sessões de treinamento} com \textit{learning rate schedules} distintos:
...
\begin{itemize}
    ...
    \item \textbf{Sessão 4} (servidor, 3 épocas com early stopping): Treino único do zero com \texttt{EarlyStoppingCallback(patience=3)}, \texttt{eval\_steps=250} e \texttt{num\_train\_epochs=3}. O early stopping interrompeu automaticamente no step 1.750, carregando o melhor checkpoint (step 1.000, \texttt{eval\_loss=0,647}). Esta sessão valida a metodologia recomendada.
\end{itemize}
...
Esta diferença é discutida na Seção~\ref{subsec:influencia-lr}. A comparação com a Sessão 4 (early stopping) é apresentada na Seção~\ref{subsec:early-stopping-resultados}.
\end{warningbox}
...
\begin{itemize}
    \item \textbf{Similaridade semântica} (apenas Trilha B): Cosine similarity entre embeddings da resposta gerada e da resposta de referência, usando o modelo E5 multilíngue. Captura equivalência de significado mesmo com formulações diferentes.
...
\subsection{Validação com Early Stopping}
\label{subsec:early-stopping-resultados}

Para validar a metodologia recomendada (treino único com early stopping), foi conduzida uma quarta sessão de treinamento (Sessão 4) do zero, com as seguintes configurações:

\begin{itemize}
    \item \texttt{num\_train\_epochs=3}, \texttt{eval\_steps=250}, \texttt{save\_steps=250}
    \item \texttt{EarlyStoppingCallback(patience=3, threshold=0.01)}
    \item \texttt{load\_best\_model\_at\_end=True}
\end{itemize}

O early stopping interrompeu automaticamente o treinamento no step 1.750, após 3 avaliações consecutivas sem melhoria significativa no \texttt{eval\_loss}. O melhor checkpoint (step 1.000, \texttt{eval\_loss=0,647}) foi carregado automaticamente.

A Tabela~\ref{tab:early-stopping-vs-1500} compara o modelo early stopping com o checkpoint-1500 (melhor modelo das sessões anteriores):

\begin{table}[H]
\centering
\caption{Comparação: Early Stopping (Sessão 4) vs Checkpoint-1500 (Sessão 3)}
\label{tab:early-stopping-vs-1500}
\begin{tabular}{lccc}
\toprule
\textbf{Métrica vs Ground Truth} & \textbf{Baseline} & \textbf{Early Stopping} & \textbf{Checkpoint-1500} \\
\midrule
BLEU                      & 0,039 & 0,222 & \textbf{0,298} \\
ROUGE-1 F1                & 0,318 & 0,441 & \textbf{0,517} \\
ROUGE-L F1                & 0,175 & 0,345 & \textbf{0,421} \\
Similaridade Semântica    & 0,904 & 0,928 & \textbf{0,939} \\
\bottomrule
\end{tabular}
\end{table}

\begin{importantbox}[Resultado: Checkpoint-1500 Permanece Superior]
Apesar da metodologia mais limpa (treino único, reprodutível), o modelo early stopping apresenta métricas \textbf{inferiores} ao checkpoint-1500 em todas as dimensões: BLEU $-26\%$ (0,222 vs 0,298), ROUGE-L $-18\%$ (0,345 vs 0,421) e similaridade semântica $-1,2\%$ (0,928 vs 0,939).
\end{importantbox}

A explicação para esta diferença reside na \textbf{herança de pesos} do checkpoint-1500:

\begin{enumerate}
    \item O checkpoint-1500 (Sessão 3) \textbf{partiu do checkpoint-1000 da Sessão 1}, que já havia passado por $\sim$0,9 épocas de treinamento com um LR schedule mais longo.
    \item Ao retomar com \texttt{max\_steps=1.500}, o cosine LR reinicia do valor máximo mas decai rapidamente --- no step 1.300, o LR já é $\sim$0,000007.
    \item O modelo early stopping (Sessão 4) partiu do \textbf{Mistral base} (sem fine-tuning prévio) e parou no step 1.000, tendo visto menos exemplos de treino com menos refinamento.
\end{enumerate}

Em termos de \textbf{steps efetivos de treinamento}, o checkpoint-1500 acumulou $\sim$2.500 steps (1.000 da Sessão 1 + 1.500 da Sessão 3), enquanto o early stopping acumulou apenas 1.000 steps --- uma diferença de 2,5$\times$.

\begin{infobox}[Implicação Prática]
Para um \textbf{novo projeto}, a abordagem com early stopping é recomendada por sua reprodutibilidade e simplicidade. No entanto, para \textbf{este projeto}, o checkpoint-1500 permanece como modelo final devido às suas métricas superiores --- resultado de uma sequência de treinamento que, embora menos elegante metodologicamente, produziu um modelo de maior qualidade.

A lição é que \textit{early stopping} é uma técnica de regularização eficaz, mas não substitui os benefícios de pesos pré-otimizados em um regime de baixo learning rate.
\end{infobox}
