...
\textbf{Validação com early stopping}: Uma sessão adicional (Sessão 4) foi conduzida do zero com \texttt{EarlyStoppingCallback}, interrompendo automaticamente no step 1.750. Embora metodologicamente mais limpa, o modelo resultante apresentou métricas inferiores ao checkpoint-1500 (BLEU 0,222 vs 0,298; ROUGE-L 0,345 vs 0,421). A diferença se explica pela herança de pesos: o checkpoint-1500 acumulou $\sim$2.500 steps efetivos de treinamento, enquanto o early stopping acumulou apenas 1.000. Para novos projetos, recomenda-se early stopping; para este projeto, o checkpoint-1500 permanece como modelo final.
\end{importantbox}
...
\begin{itemize}
    ...
    \item \textbf{LR schedule como hiperparâmetro crítico}: A análise das três sessões de treinamento (Seção~\ref{subsec:influencia-lr}) revelou que o comportamento do modelo no mesmo step varia drasticamente conforme o \texttt{max\_steps}. Um modelo com 1.500 steps converge suavemente onde um com 5.780 colapsa, devido à diferença no learning rate.
    ...
    \item \textbf{Treino único para reprodutibilidade}: Checkpoints de sessões distintas são difíceis de reproduzir e comparar diretamente. A Sessão 4 validou a abordagem de treino único com early stopping (\texttt{03\_fine\_tuning\_early\_stopping.ipynb}), que é recomendada para novos projetos. No entanto, a herança de pesos de sessões anteriores pode produzir modelos superiores, como demonstrado pelo checkpoint-1500.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Fine-tuning com mais dados}: O modelo atual foi treinado com $\sim$20.500 pares QA. Expandir o dataset com mais fontes médicas em português nativo (em vez de tradução automática) e guidelines atualizados poderia melhorar a qualidade das respostas.
